...
export default function MainLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-subtle text-foreground selection:bg-accent/10 selection:text-foreground">
      ...
      >
        <div className="flex h-[4.5rem] items-center justify-between border-b border-border px-5">
          ...
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-xl text-muted transition-colors hover:bg-surface-raised hover:text-foreground"
          >
            <iconify-icon
      ...
      <aside
        className="sticky top-0 hidden h-screen w-64 flex-col border-r border-border bg-surface md:flex"
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex min-h-16 items-center justify-between gap-4 border-b border-border bg-subtle/95 px-6 py-3 backdrop-blur sm:px-8 lg:px-10">
          <div className="flex min-w-0 items-center gap-3">
            <button
              type="button"
              className="flex h-10 w-10 items-center justify-center rounded-xl text-muted transition-colors hover:bg-surface hover:text-foreground md:hidden"
            ...
            <div className="relative min-w-0">
              <button
                ...
                className="flex max-w-[16rem] items-center gap-3 rounded-xl border border-border bg-surface px-3 py-2 text-left transition-colors hover:bg-surface-raised"
                aria-haspopup="menu"
                    ...
                    {workspaces === undefined ? (
                      ...
                      workspaces.map((workspace) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`mt-0.5 flex h-9 w-9 items-center justify-center rounded-full text-sm font-semibold ${active ? "bg-accent text-accent-fg" : "bg-surface-raised text-muted"}`}
                            >
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="mt-1 text-xs text-muted capitalize">
                                {workspace.role} · แพ็กเกจ {workspace.plan}
                              </p>
                            </div>
                            {active && (
                              <span
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={toggleTheme}
              className="flex h-10 items-center gap-2 rounded-xl border border-border bg-surface px-3 text-sm font-medium text-foreground transition-colors hover:bg-surface-raised"
              aria-label={`สลับเป็นโหมด${theme === "dark" ? "สว่าง" : "มืด"}`}
            >
              <iconify-icon
                icon={theme === "dark" ? "solar:sun-2-bold-duotone" : "solar:moon-bold-duotone"}
                width="18"
                height="18"
                aria-hidden="true"
            ...
            <Link
              href="/profile"
              className="flex shrink-0 items-center gap-3 rounded-xl px-3 py-2 transition-colors hover:bg-surface"
            ...
            >
              ...
              <div className="hidden text-left sm:block">
                <p className="text-sm font-semibold leading-none text-foreground">
                  {currentUser?.name ?? "กำลังโหลดโปรไฟล์"}
                </p>
                <p className="mt-1 text-xs leading-none text-muted capitalize">
                  {profileRoleLabel}
                </p>
              </div>
            </Link>
        ...
        <main
          id="main-content"
          className="flex-1 overflow-y-auto px-6 py-8 sm:px-8 lg:px-10"
          tabIndex={-1}
        >
          {children}
        </main>
...
function NavItem({
  ...
  return (
    <Link
      ...
      className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-colors ${isActive ? "bg-accent-subtle text-foreground" : "text-muted hover:bg-surface-raised hover:text-foreground"}`}
    >
      <iconify-icon
        icon={icon}
        width="20"
