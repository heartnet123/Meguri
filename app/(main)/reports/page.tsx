    ...
    <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-muted mt-2 leading-relaxed max-w-xl">
            สร้างรายงานเชิงลึก ตั้งเวลาส่งออกข้อมูลอัตโนมัติ และตรวจสอบข้อมูลย้อนหลังได้จากที่เดียว
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {reports.map((report) => (
          <div
            key={report.id}
            className="group bg-surface border border-border rounded-2xl shadow-sm hover:shadow-2xl hover:shadow-accent/5 hover:border-accent/20 transition-all duration-500 flex flex-col h-full overflow-hidden relative"
          >
            <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
              <span className="text-[10px] font-black uppercase tracking-widest text-muted/20">
                {report.id}
              </span>
            </div>
            ...
            <div className="p-7 flex-1 flex flex-col">
              <div className="flex items-start justify-between mb-6">
                <div
                  className={`w-14 h-14 rounded-2xl ${report.bgColor} flex items-center justify-center border border-border group-hover:scale-110 transition-transform duration-500 shadow-sm shadow-accent/5`}
                >
                  <iconify-icon
                    icon={report.icon}
                    width="28"
              ...
              <p className="text-sm text-muted/80 leading-relaxed mb-8 flex-1 line-clamp-3 font-medium">
                {report.description}
              </p>
