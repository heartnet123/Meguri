...
export default function NewOrderPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<
    "cash" | "credit_card" | "mobile_pay" | "invoice"
  >("cash");
  ...
  const updateQuantity = useCallback((recipeId: Id<"recipes">, quantity: number) => {
    if (quantity <= 0) {
      ...
      setCart((prev) => prev.map((c) => (c.recipeId === recipeId ? { ...c, quantity } : c)));
    }
    setError(null);
  }, []);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500 mt-0.5">
              เลือกสินค้าจากแคตตาล็อก ระบบจะตัดสต็อกตามสูตรสินค้าอัตโนมัติ
            </p>
          </div>
        </div>
        {cart.length > 0 && (
          ...
          >
            {isLoading ? (
              Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="bg-white border border-neutral-100 rounded-md p-4 animate-pulse"
                >
                    ...
                    impact.ingredientImpacts.map((item: any) => (
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <span
                            className={`w-1.5 h-1.5 rounded-full shrink-0 ${item.insufficient ? "bg-red-500" : "bg-emerald-500"}`}
                          />
                          <span
                            className={`font-medium truncate ${item.insufficient ? "text-red-700" : "text-neutral-900"}`}
                          >
                            {item.name}
                          </span>
                        </div>
                        <div className="text-right shrink-0 ml-3">
                          ...
                          <span className="text-neutral-400 mx-1">=</span>
                          <span
                            className={`font-semibold ${item.insufficient ? "text-red-700" : "text-neutral-900"}`}
                          >
